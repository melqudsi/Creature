...
- **Layout source of truth:** `scripts/world/memphis_layout.gd` (`MemphisLayout`). 160×112 tiles. Region rects (first match wins), road rects (visual only, walkable), river/bridge/Mud-Island geometry, and per-region scatter specs all live there. `GameConfig.region_for_tile()` checks sub-zones (Dump/BBQ/Bus Stop) first, then delegates.
- **Determinism is load-bearing:** scattered houses/trees/towers come from a fixed-seed RNG (`SCATTER_SEED`). Every client must build identical blocked tiles or pathfinding and the smoker's near-house check disagree across players. Never scatter with unseeded `randi()`.
- **`GameState.blocked_tiles` is a Dictionary** (Vector2i → true), not an Array — the river alone is ~1,600 blocked tiles and `tile in array` scans made A* unusable. `GridNav` takes the Dictionary directly.
- **Elevation trick:** all walkable ground stays at y=0; the river is a water plane at y=-0.45 with a bank wall (Memphis bluff). The bridge deck top is ~y=0.02 with its own StaticBody so taps on it resolve to tiles. Ground overlays are height-layered to avoid z-fighting: region tints 0.018–0.030, interstates 0.040, streets 0.044, landfill tint 0.05, blood splats ~0.06, click marker 0.09.
- **Old world relocation:** the 32×24 world sits at `OLD_WORLD_OFFSET` (+20,+80) inside South Memphis. `interactive_objects()` / `money_seed_objects()` / `trash_pile_tiles()` apply the offset via `_old()`. Legacy creature rows (x<32 AND y<24) are remapped **for the local player only** in `db_row_to_player_data` — remotes self-heal because their own client remaps + flushes on spawn/60s heartbeat. Shared `world_objects` rows were wiped + re-seeded post-deploy (admin **Reset ALL World Objects**).
- **Water safety:** anything that drops/scatters money must go through `GameConfig.safe_drop_tile()` (clamps to map, nudges out of water via `GridNav.nearest_walkable`) — money in the river would be unreachable forever.
- **Path cost:** worst-case cross-map path ≈ 220ms (one-time hiccup per extreme click); normal camera-range clicks ≤ 1ms.
- **Roads are two-lane:** every road rect is 2 tiles wide with a painted center divider; street names are flat `Label3D`s on the asphalt (`world_map.gd::_add_road_labels`, repeated every ~26 tiles). Ground overlay heights are staggered by kind AND orientation (interstate horizontal 0.036 / vertical 0.038, street 0.044/0.046, dividers +0.004, labels 0.065) so crossings never z-fight.
- **NPC traffic (`npc_traffic.gd`):** client-local ambient Altimas + buses on every road (right-hand lanes = ±0.5 tiles from the divider, flipping with direction; u-turn at ends via a position lerp so the lane swap arcs). Kill checks reuse `FormDefs.resolve_player_death` against the LOCAL player only — same networking rule as all kills. Traffic is not synced and doesn't need to be.
- **Occlusion fade:** `world_map.gd::_update_occlusion_fades()` (10Hz) fades `WorldObject`s of kind building/tree whose XZ position is within `radius + 0.45` of the camera→player sight line, provided the ray height at that point is below `WorldObject.occlusion_height()`. Fading flips `StandardMaterial3D.transparency` + albedo alpha and turns the mesh's shadow off (`WorldObject.set_occlusion_faded`).
